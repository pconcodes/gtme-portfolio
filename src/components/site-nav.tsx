...
export function SiteNav() {
  return (
    <header className="sticky top-0 z-20 border-b border-border-soft bg-ink/85 backdrop-blur">
      <nav className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
        <Link
          href="/"
          className="flex items-center gap-2.5 font-mono text-sm font-medium tracking-widest text-text"
        >
        ...
        <ul className="hidden items-center gap-8 font-mono text-xs uppercase tracking-widest text-text-muted md:flex">
          {navLinks.map((l) => (
            <li key={l.href}>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/contact"
            className="rounded-[2px] border border-accent/40 bg-accent/10 px-4 py-2 font-mono text-xs uppercase tracking-widest text-accent-soft transition-colors hover:bg-accent/20"
          >
            Get in touch
          </Link>
        </div>
